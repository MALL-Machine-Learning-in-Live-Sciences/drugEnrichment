...
def correlation_drugs(df_drugs, df_counts):  
    ...
    correlation_matrix = pd.DataFrame(index=df_drugs.columns, columns=df_counts_filled.columns)

    for drug_col in df_drugs.columns:
        auc_v = df_drugs[drug_col].values
        for gene in df_counts_filled.columns:
            tf_v = df_counts_filled[gene].values

    ...
    if organ in type_list:
        ...
        try:
            if len(df_sub_outputs)>10 and len(df_sub_tf)>10:
                gsea_result, pvalues, cor_matrix = DE.run_drug_enrichment(df_drugs=df_sub_outputs, df_counts=df_sub_tf, drug_net=drug_net, shared_elements='name', group='moa', min_elements=5, number_of_threads=-1, return_cormatrix=True)
